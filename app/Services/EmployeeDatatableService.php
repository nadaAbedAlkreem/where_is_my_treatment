...
class EmployeeDatatableService extends Controller
{
            ...
            ->filter(function ($query) use ($request) {
                ...
                if (!empty($request->get('search_employee')) ) {
                    ...
                    $query->where(function ($query) use ($employee) {
                        ...
                        $query->orWhere('email', 'like', '%' . $employee . '%');
                        $query->orWhere('phone', 'like', '%' . $employee . '%');
                    });
...
//              })
                      ...
                      <!--begin::Action=-->
                            ...
                            <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-125px py-4" data-kt-menu="true">
                                ...
                                <div class="menu-item px-3">
                                    <a data-bs-toggle="modal" data-bs-target="#kt_modal_update_employee" data-id="'.$data->id.'" data-name="'.$data->name.'" data-email="'.$data->email.'"  data-phone="'.$data->phone.'"  class="menu-link px-3 updateRe">تعديل</a>
                                </div>
                                <!--end::Menu item-->
                                <!--begin::Menu item-->
                                <div class="menu-item px-3">
            ...
            ->addColumn('pharmacyOwner', function ($data)
            {
                return '<div class="badge badge-light fw-bolder">'.$data->parent->name.'</div>' ;
            })
            ...
            ->rawColumns([ 'action'  , 'pharmacyOwner' , 'name' , 'email' ,'status', 'phone' , 'checkbox' , 'created_at' ])
            ->make(true);
